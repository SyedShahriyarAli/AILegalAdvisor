...
def detect_sections_and_chapters(pages_data: List[Dict]) -> List[Dict]:
    ...
    chapter_pattern = re.compile(r"^\s*(CHAPTER[\s\-]+[A-Z0-9]+)\s*$", re.IGNORECASE)
    ...
    for page_data in pages_data:
        ...
        for i, line in enumerate(lines):
            ...
            if chapter_match:
                ...
                # Try to get the chapter title from the next line if it's not a section
                chapter_title = ""
                if i + 1 < len(lines):
                    next_line_text = lines[i+1].get("text", "").strip()
                    if not section_pattern.match(next_line_text) and not chapter_pattern.match(next_line_text):
                        chapter_title = next_line_text
                
                if chapter_title:
                    current_chapter = f"{chapter_id} {chapter_title}"
                else:
            ...
            # A simple heuristic: if it's before page 4, it might be CONTENTS.
            # ETO-2002 Act text starts on page 4.
            if page_num < 4:
                continue
...
def parse_eto_2002(pdf_path: str) -> Dict:
    ...
    output = {
        "document": {
            ...
            "year": 2002,
            "source_file": "The Electronic Transactions Ordinance, 2002.pdf",
...
if __name__ == "__main__":
    ...
    pdf_path = os.path.join(script_dir, "../../data/pdfs/ETO-2002.pdf")

    if not os.path.exists(pdf_path):
        print(f"Error: PDF file not found at {pdf_path}")
        sys.exit(1)

    print("Starting ETO 2002 parsing...")
    result = parse_eto_2002(pdf_path)

    output_path = os.path.join(script_dir, "../../data/jsons/electronic-transactions-ordinance-2002.json")
