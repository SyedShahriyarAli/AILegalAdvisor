...
def detect_articles(pages_data: List[Dict]) -> List[Dict]:
    """Detect articles with proper title extraction (supports multi-line titles)"""
    articles = []
    # article_pattern = re.compile(r'^\s*[\[\(]?\s*(\d+[A-Z]*)\.\s+(.*)')
    # NEW (allow up to 3 leading “[”, optional “(”, and optional space after the dot)
    article_pattern = re.compile(r'^\s*(?:\[\s*){0,3}\(?\s*(\d+[A-Z]*)\s*\.\s*(.*)')

    current_part = None
    current_chapter = None

    BOLD_MIN_FOR_TITLE = 0.18  # Require at least this much bold to treat a line as a title line
    ...
    def line_metrics(ln: Dict) -> Dict[str, Any]:
        ...
        titlecase_ratio = (sum(1 for w in words if (w.isupper() or (w[0].isupper() and w[1:].islower()))) / len(words)) if words else 0.0
        return {
            'text': txt,
            'top': ln.get('top', 0.0),
            'x0': ln.get('x0', 0.0),
    ...
    def looks_parenthetical_heading(txt: str) -> bool:
        t = txt.strip()
        return t.startswith('(') and t.endswith(')') and len(t) <= 60 and not re.search(r'\d', t)
        ...
        for i, line in enumerate(lines):
            ...
            def is_probable_title(candidate: Dict[str, Any]) -> bool:
                ...
                strong_heading = (
                    ...
                    or candidate['caps_ratio'] >= 0.60
                    or candidate['titlecase_ratio'] >= 0.60
                )

                # Sentence-like checks to avoid previous paragraph tails
                sentence_like = bool(re.search(r'\b(shall|may|be|is|are|was|were|has|have|had|will)\b', txt, re.I))
                ...
                if not strong_heading:
                    ...
                    if 2 <= wc <= 18 and not ends_with_sentence_punct and not sentence_like:
                        return True
            ...
            for k in range(i - 1, max(-1, i - 1 - max_lines_back), -1):
                ...
                bold_only = _line_bold_text(prev) if not is_parenthetical else pm['text']
                text_to_use = bold_only if bold_only else pm['text']
                title_lines.append({**pm, 'text_to_use': text_to_use})

            if title_lines:
                article_title = ' '.join(t['text_to_use'] for t in reversed(title_lines)).strip()
                article_title = re.sub(r'\s+', ' ', article_title)
            else:
                first_sentence = article_first_line.split('.')[0].strip() if '.' in article_first_line else article_first_line
                if len(first_sentence) < 200 and not first_sentence.startswith('('):
                    article_title = first_sentence
                else:
                    article_title = first_sentence[:100] + "..." if len(first_sentence) > 100 else first_sentence
...
def extract_article_content(pages_data: List[Dict], articles: List[Dict]) -> List[Dict]:
    ...
    if not articles:
        return []
    sorted_articles = sorted(articles, key=lambda x: (x['page_number'], x['line_index']))
    ...
    for i, article in enumerate(sorted_articles):
            ...
            for j in range(start_idx, end_idx):
                if j < len(filtered_lines):
                    ...
                    if current_page_num == page_num and j == line_index:
                        header_match = re.match(r'^\s*(?:\[\s*){0,3}\s*\d+[A-Z]*\s*\.\s*(.*)$', line_text)
                        if header_match:
                    ...
                    next_hdr = re.match(r'^\s*(?:\[\s*){0,3}\s*\d+[A-Z]*\s*\.\s*', line_text)
                    if (line_text.startswith('PART ') or 
                        line_text.startswith('CHAPTER ') or
                        (next_hdr and current_page_num == end_page and j >= end_line_index)):
                        break
        ...
        article['clauses'] = extract_clauses(cleaned_content)
...
def clean_article_content(content: str) -> str:
    ...
    cleaned = re.sub(r'(?mi)^\s*\d+\s*$', '', cleaned)  # page numbers on their own lines
...
def extract_clauses(content: str) -> List[Dict]:
    """
    Extract only top-level numeric clauses:
    - Starts like (1) ... or [(1)] ...
    - Not mid-text references like 'clauses (1) and (2)' (require non-word boundary before marker)
    - Each clause spans until the next marker or end.
    """
    clauses: List[Dict] = []
    if not content:
        return clauses

    # Find clause markers anywhere, but only if preceded by start/newline/punctuation (not a letter/number/_)
    marker = re.compile(r'(?s)(?<!\w)(?:\[\s*)?\(\s*(\d+)\s*\)\]?\s+(?=[A-Z"\'])')
    ...
    for idx, m in enumerate(matches):
        cid = m.group(1)
        start = m.end()  # start of clause text after "(n)"
...
def validate_and_order_articles(articles: List[Dict]) -> List[Dict]:
    ...
    print(f"Found articles numbered: {sorted(set(article_numbers))[:20]}...")  # Show first 20
...
def detect_schedule_amendments(pages_data: List[Dict]) -> List[Dict]:
    ...
    article_ref_pat = re.compile(r'^\s*\[\s*Article\s+(\d+[A-Z]?)\s*(?:\([^)]+\))?\s*\]\s*$', re.IGNORECASE)
    schedule_banner_pat = re.compile(r'^\s*\[\s*[A-Z][A-Z\s\-]*SCHEDULE\s*\]\s*$', re.IGNORECASE)
    ...
    while page_idx < total_pages:
        ...
        while i < len(lines):
            ...
            while p < total_pages:
                ...
                if j < len(curr_lines) and article_ref_pat.match((curr_lines[j].get('text') or '').strip()):
                    break
                # Move to next page
                p += 1
                j = 0
            ...
            if p == page_idx:
                ...
                lines = _filter_page_lines(pages_data[page_idx]) if page_idx < total_pages else []
                i = j
...
def apply_schedule_amendments(articles: List[Dict], schedule_amendments: List[Dict]) -> List[Dict]:
    """
    ...
    for amend in schedule_amendments:
        ...
        idx = sum(1 for c in existing if str(c.get('id', '')).startswith('schedule-amendment-')) + 1
        existing.append({
            'id': f'schedule-amendment-{idx}',
            'text': text
        })
...
def parse_constitution(pdf_path: str) -> Dict:
    ...
    output = {
        'document': {
            ...
            'year': 1973,
            'source_file': 'Constitution, 1973.pdf',
...
def determine_context(structure_elements: List[Dict], page_num: int, y_pos: float) -> Optional[str]:
    """Determine which chapter/part an article belongs to"""
    # Find the most recent chapter/part before this article
    candidates = [elem for elem in structure_elements 
                 if elem['page'] < page_num or (elem['page'] == page_num and elem['y_position'] < y_pos)]
...
def validate_parsing_result(parsed_data: Dict) -> Dict:
    ...
    article_numbers = [int(a['article_number']) for a in articles if a['article_number'].isdigit()]
    ...
    validation_report = {
        ...
        'article_numbers_range': (min(article_numbers) if article_numbers else 0, 
    ...
    if missing_articles:
        validation_report['validation_notes'].append(f"Missing articles: {sorted(missing_articles)[:10]}...")
...
if __name__ == "__main__":
    ...
    pdf_path = backend_dir / "data" / "pdfs" / "Constituion, 1973.pdf"
    if not pdf_path.exists():
        raise FileNotFoundError(f"PDF not found: {pdf_path}")
    
    # Parse the document
    result = parse_constitution(str(pdf_path))
    ...
    # Save results
    output_path = backend_dir / "data" / "jsons" / "constitution-1973.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(result, f, indent=2, ensure_ascii=False)
    
    print(f"Parsing completed successfully! Saved to: {output_path}")